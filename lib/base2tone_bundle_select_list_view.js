...
class Base2ToneBundleSelectListView extends SelectListView {
  ...
  viewForItem(theme) {
    ...
    if (this.base2tone.isConfigTheme(theme.scheme, theme.contrastGutter, theme.invertColors)) {
  ...
  selectItemView(view) {
    ...
    if (this.attached) { return this.base2tone.enableTheme(theme.scheme, theme.contrastGutter, theme.invertColors); }
  }
  ...
  confirmed(theme) {
    ...
    this.base2tone.setThemeConfig(theme.scheme, theme.contrastGutter, theme.invertColors);
    this.cancel();
  ...
  getThemes() {
    ...
    const invertColors = atom.config.get(`${this.base2tone.packageName}.invertColors`);
    const themes = [];
